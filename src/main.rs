...
use catalyst_rsx::routing::handlers::{dashboard_handler, admin_login_page};
...
async fn main() -> std::io::Result<()> {
    ...
    let config = Config::from_env()
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;    let mut builder = SslAcceptor::mozilla_intermediate(SslMethod::tls())?;
